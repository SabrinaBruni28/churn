# Cáculo de Churn com Média Simples
...
churn = 1 - df_id_churn.mean(axis=1)
churn = pd.DataFrame(churn)
churn.insert(0,'id',cdf['id_cliente'].unique().astype(str))
churn.rename(columns={0: 'churn'}, inplace=True)

churn.to_csv('calculoChurnMediaSimples.txt', index=False, header=True, quoting=csv.QUOTE_NONNUMERIC)