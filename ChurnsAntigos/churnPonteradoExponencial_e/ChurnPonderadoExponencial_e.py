# Cáculo de Churn Ponderado Exponencial com base e
...
for i in range(len(dates_vector)-1):
    ...
    for j in range(len(cdf['id_cliente'])):
        if ((dates_vector[i] <= cdf.loc[j,'date']) and (cdf.loc[j,'date'] < dates_vector[i+1])):
            mat_id_churn[cdf.loc[j, 'id_cliente']-1,i] = e**(i+1)
...
churn.to_csv('calculoChurnPonderadoExponencial_e.txt', index=False, header=True, quoting=csv.QUOTE_NONNUMERIC)